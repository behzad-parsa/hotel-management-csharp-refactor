...
{
    public partial class OrderFoodControl : UserControl
    {
        ...
        private void LoadBookingData()
        {
            string query = "Select  res.id As ResID,  rn.Title As RoomNo , Firstname + ' ' + Lastname As Name  ,StartDate as CheckIn , EndDate as CheckOut , res.DateModified as Date " +
              ...
              " res.startDate <= '" + DateTime.Now.Date + "' And res.EndDate >= '" + DateTime.Now.Date + "' And res.CancelDate IS Null And r.BranchID = " + Current.CurrentUser.BranchID + " AND res.ID Not IN (Select b.ResID From Bill b)";
        ...
        { 
            string query = "Select Distinct orderf.ID as OrderID , Firstname + ' ' + Lastname +' - '+ rn.Title AS BookDetail  , f.Title  , Count As Qty  , f.Price , Total , orderf.DateModified As Date " +
                ...
                " Where orderf.FoodID = f.ID And res.CustomerID = cus.ID And cus.ActID = a.ID And res.RoomID = r.ID And r.RoomNumberID = rn.ID And orderf.ResID = res.ID  And r.BranchID = " +Current.CurrentUser.BranchID;
                ...
                using (ActionFood actionFood = new ActionFood(ActionFood.Action.Add))
                {
                    ...
                    if (actionFood.completeActionFlag)
                    {
                        ...
                        Current.CurrentUser.Activities.Add(
                            ...
                            "New Food has been submited to menu by " + Current.CurrentUser.Firstname + " " + Current.CurrentUser.Lastname)
                            );
                    }
                }
            }
            ...
            else
            {
                ...
                using (Panel p = new Panel())
                {
                    ...
                    using (ActionFood actionFood = new ActionFood(ActionFood.Action.Edit))
                    {
                        ...
                        if (actionFood.completeActionFlag)
                        {
                            Current.CurrentUser.Activities.Add(new Activity("Edit a Food", "a Food has been edited by " + Current.CurrentUser.Firstname + " " + Current.CurrentUser.Lastname));
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                var dilogResult = MessageBox.Show("Are You Sure You Want To Delete This Record ?", "Delete", 
                    MessageBoxButtons.YesNo, MessageBoxIcon.Question);

                if (dilogResult == DialogResult.Yes)
                {
                    var resultDelete = _foodService.DeleteFood(FoodID);
                    if (resultDelete)
                    {                       
                        LoadFoodData();
                        dgvFood.ClearSelection();
                        Current.CurrentUser.Activities.Add(
                            new Activity("Delete a Food", "Food has been deleted by " + Current.CurrentUser.Firstname + " " + Current.CurrentUser.Lastname)
                            );
                    }
                    else
                    {
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            if (doubleClickDGVBookingFlag && doubleClickDGVBookingFlag)
            {
                var orderFood = new OrderFood()
                {
                    FoodID = FoodID,
                    ResID = ResID,
                    Count = numQuantity.Value,
                    Total = Convert.ToInt32(lblTotal.Text),
                    DateModified = DateTime.Now
                };
                var resultInsert = _orderFoodService.InsertOrderFood(orderFood);
                if (resultInsert)
                {
                    ...
                    Current.CurrentUser.Activities.Add(
                        new Activity("Submit New Food Order", "New Food order has been submited by " + Current.CurrentUser.Firstname + " " + Current.CurrentUser.Lastname)
                        );
                }
                else
                {
        ...
        private void btnDeleteOrder_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    var resultDelete = _orderFoodService.DeleteOrderFood(OrderFoodID);
                    if (resultDelete)
                    {
                        LoadOrderData();
                        dgvOrder.ClearSelection();
                        Current.CurrentUser.Activities.Add(
                            new Activity("Delete Food Order", "an Order from Food List has been deleted by " + 
        ...
        private void btnReportFood_Click(object sender, EventArgs e)
        {
            if (foodData != null)
            {
                Current.CurrentUser.Activities.Add(
                    ...
                    "the report from the dood menu has been created by " + Current.CurrentUser.Firstname + " " + Current.CurrentUser.Lastname)
                    );
